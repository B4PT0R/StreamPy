...
Quite similarly to a Jupyter Notebook, just type your python commands in the input cell below and click "Run" to get the results.\n 
StreamPy features a special streamlit_deferrer module to allow handling (almost) all Streamlit functions and syntaxes for a seamless integration in the StreamPy interactive console.\n
...
(Note: st is a st_deferrer class instance, not streamlit itself. So don't import streamlit as st or it will overwrite the prefix with normal streamlit module and break the console's functionalities...)\n
Try to run the following snippet, demonstrating the basic features of streamlit:
```python
...
```\n
It creates 3 columns, places a text_input widget in the first, a button in the second that triggers the writing of the text content in an empty placeholder in the third column.
Type some text and click the button to see what happens.\n                 
Note that txt is an object placeholder for the (future) content of the text input box, and will be actualized in real-time if the content changes.
...
Refer to [Streamlit documentation](https://docs.streamlit.io/library/api-reference) to get more informations on possible commands and how to use them.\n
The project is mostly working but still a prototype and you may run into errors or undesired behaviour. If you want to report a bug or feel like contributing to the project, feel free to check the [GitHub repository](https://github.com/B4PT0R/StreamPy) or reach me out directly at bferrand.maths@gmail.com.\n
Happy testing!  
""")
